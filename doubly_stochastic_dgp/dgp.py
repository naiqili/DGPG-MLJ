...
from dgp_graph.weighted_dataholder import wMinibatch, wpMinibatch
...
class TWDGP_Base(Model):
    """
    The base class for Deep Gaussian process models.
    Implements a Monte-Carlo variational bound and convenience functions.
    """
    def __init__(self, X, Y, time_vec, likelihood, layers,
                 minibatch_size=100,
                 num_samples=1, num_data=None, wfunc='exp',
                 **kwargs):
        Model.__init__(self, **kwargs)
        self.num_samples = num_samples
        ...
        if np.ndim(X) == 2:
            self.num_data = num_data or X.shape[0]
            self.X = wMinibatch(X, time_vec, batch_size=minibatch_size, seed=0, wfunc=wfunc)
            self.Y = wMinibatch(Y, time_vec, batch_size=minibatch_size, seed=0, wfunc=wfunc)
        else:
            self.num_data = num_data or X.shape[1]
            self.X = wpMinibatch(X, time_vec, batch_size=minibatch_size, seed=0, wfunc=wfunc)
            self.Y = wpMinibatch(Y, time_vec, batch_size=minibatch_size, seed=0, wfunc=wfunc)
            self.m = 4
